...
class PageGenerator:
    ...
    def _build_system_prompt(self, include_charts, include_icons, role: SlideRole, placeholders):
        ...
        content_rules = """
        КОНТЕНТ (закон пирамиды Минто):
        1. Заголовок = вывод, а не тема. Плохо: "Рыночный анализ". Хорошо: "Рынок растёт на 23% — мы лидеры".
        2. Каждый bullet = утверждение + доказательство. Плохо: "Много конкурентов". Хорошо: "5 конкурентов → мы дифференцируемся по скорости (2x быстрее)".
        3. Иерархия: заголовок → 3-5 тезисов → подтверждение для каждого.
        4. Не более 6 слов в строке, не более 4 строк в блоке.
        5. Используй силовые слова: "увеличивает", "сокращает", "гарантирует", "единственный".
        """

        prompt = f"""{content_rules}
Ты — профессиональный дизайнер слайдов. Размести контент в заданной сетке.
...
ВИЗУАЛЬНАЯ ИЕРАРХИЯ:
- Заголовок — самый крупный текст, всегда в верхней трети слайда
- Ключевое число/метрика — в 2x размере обычного текста, цвет $accent
- Иконки-маркеры — строго 40x40 px, цвет $accent
- Декоративные элементы — прозрачность 10-20%, цвет $primary
- Белое пространство = минимум 30% площади слайда
- Контраст: тёмный текст на светлом фоне или наоборот
